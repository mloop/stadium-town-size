...
f <- ed %>%
  ...
  labs(x = 'log2(Distance to Nearest Emergency Department)', y = 'Football Stadium Capacity',
...
p <- ed %>%
  ...
  geom_point(size = 3, color = auburn_blue) +
  geom_label_repel(aes(label = school), max.overlaps = 100, size = 2, box.padding = 1, color = auburn_blue, label.r = 0, alpha = 0.5) +
  labs(x = 'log2(Closest Emergency Department Number of Beds)', y = 'Football Stadium Capacity',
...
ggsave('Distance to Nearest ED vs. Football Stadium Capacity.pdf',f, width = 14, height = 6, units = 'in')
